...
const NIVEL_INFO = {
  '1': {
    label: 'Desactivado',
    desc: 'El bot no responde. Agustín atiende todo manualmente por WhatsApp.',
    badge: 'text-se-text',
  },
  '2': {
    label: 'Mensajes automáticos',
    desc: 'Solo envía mensajes programados: recordatorios, promos y reactivaciones. No agenda turnos.',
    badge: 'text-sw-text',
  },
  '3': {
    label: 'Bot completo',
    desc: 'Bot conversacional activo: agenda turnos, confirma, recuerda, envía promos y reactiva clientes.',
    badge: 'text-ss-text',
  },
}
...
const MENSAJES_INFO = [
  {
    ...
    label: 'Bienvenida',
    trigger: 'Primer mensaje de un cliente nuevo',
    nivel: '3',
    placeholder: '¡Hola {nombre}! 👋 Soy el bot de AlPunto Barbería.\nTocá los botones y te guío por todo en segundos. 👇',
  },
  {
    key: 'mensaje_ausencia',
    label: 'Fuera de horario',
    trigger: 'Mensaje recibido fuera del horario de atención',
    nivel: '2',
    placeholder: '¡Hola! 👋 Ahora estamos cerrados.\nNuestro horario es lunes a sábado de 9 a 20hs.\nEn cuanto abramos te atendemos. ✂️',
  },
  {
    key: 'mensaje_confirmacion_turno',
    label: 'Confirmación de turno',
    trigger: 'Inmediato al confirmar un turno',
    nivel: '3',
    placeholder: '✅ ¡Turno confirmado, {nombre}!\n📅 {fecha} a las {hora}\n✂️ {servicio} con {barbero}\n\nTe esperamos en AlPunto. ¡Cualquier cambio escribinos!',
  },
  {
    key: 'mensaje_recordatorio_24h',
    label: 'Recordatorio 24hs',
    trigger: 'Automático 24hs antes del turno (CRON-1)',
    nivel: '2',
    placeholder: '¡Hola {nombre}! 👋 Te recordamos que mañana tenés turno:\n📅 {fecha} a las {hora}\n✂️ {servicio} con {barbero}\n\n¿Lo confirmamos? Respondé Sí o No.',
  },
  {
    key: 'mensaje_nudge_confirmacion',
    label: 'Nudge de confirmación',
    trigger: 'Si el cliente no confirma su turno en 5 minutos (CRON-2)',
    nivel: '2',
    placeholder: '¡Falta un paso nomás! 🙌 Tenés este turno listo para confirmar:\n📅 {fecha} a las {hora}\n✂️ {servicio} con {barbero}\n\n¿Lo confirmamos?',
  },
  {
    key: 'mensaje_reactivacion',
    label: 'Reactivación / Promo',
    trigger: 'Cuando se acerca la fecha estimada de próxima visita del cliente',
    nivel: '2',
    placeholder: '¡Hola {nombre}! ✂️ Hace un tiempo que no pasás por AlPunto.\n¿Querés sacar un turno esta semana? Tenemos horarios disponibles.',
  },
]
...
interface Bloqueado { telefono: string; motivo: string | null; bloqueado_por: string | null; created_at: string }
type Tab = 'general' | 'mensajes'
...
export default function ConfiguracionPage() {
  ...
  useEffect(() => {
    Promise.all([fetch('/api/configuracion'), fetch('/api/bloquear')])
      .then(async ([rCfg, rBloq]) => {
        ...
        setCfg(prev => ({ ...prev, ...dataCfg.config }))
  ...
  const nivelActual = NIVEL_INFO[cfg.bot_nivel as keyof typeof NIVEL_INFO] ?? NIVEL_INFO['3']

  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Header + Tabs */}
      <div className="px-6 pt-6 pb-0 border-b border-outline-variant">
        <h1 className="text-lg font-bold text-text-p mb-0.5">Configuración</h1>
        <p className="text-[11px] text-text-m mb-4">
          Bot activo: <span className={`font-semibold ${nivelActual.badge}`}>Nivel {cfg.bot_nivel} — {nivelActual.label}</span>
        </p>
        <div className="flex gap-1">
          {([['general', 'General'], ['mensajes', 'Mensajes automáticos']] as [Tab, string][]).map(([key, label]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`px-4 py-2 text-[12px] font-semibold rounded-t-lg transition-colors border-b-2 -mb-px
                ${tab === key
                  ? 'text-stitch-primary border-stitch-primary bg-surface-card'
                  : 'text-text-m border-transparent hover:text-text-p'}`}
            >
              {label}
            </button>
          ))}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-xl mx-auto px-6 py-8">
                      ...
                      <label key={n} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-[12px] font-semibold text-text-p">Nivel {n}</span>
                            <span className={`text-[10px] font-semibold ${info.badge}`}>{info.label}</span>
                          </div>
                          <div className="text-[11px] text-text-s mt-0.5">{info.desc}</div>
                        </div>
                      </label>
                    )
                  })}
          ...
          {tab === 'mensajes' && (
            <>
              <p className="text-[11px] text-text-m mb-6 leading-relaxed">
                Editá el texto de cada mensaje automático. Usá <code className="bg-surface-container px-1 rounded text-stitch-primary">{'{nombre}'}</code>, <code className="bg-surface-container px-1 rounded text-stitch-primary">{'{fecha}'}</code>, <code className="bg-surface-container px-1 rounded text-stitch-primary">{'{hora}'}</code>, <code className="bg-surface-container px-1 rounded text-stitch-primary">{'{servicio}'}</code>, <code className="bg-surface-container px-1 rounded text-stitch-primary">{'{barbero}'}</code> como variables.
              </p>

              <div className="flex flex-col gap-6">
                {MENSAJES_INFO.map(m => {
                  const nivelInfo = NIVEL_INFO[m.nivel as keyof typeof NIVEL_INFO]
                  const activo = parseInt(cfg.bot_nivel) >= parseInt(m.nivel)
                  return (
                    <div key={m.key} className={`rounded-xl border p-4 transition-opacity ${activo ? 'bg-surface-card border-outline-variant' : 'bg-surface-container/30 border-outline-variant/40 opacity-50'}`}>
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        <div>
                          <p className="text-[12px] font-semibold text-text-p">{m.label}</p>
                          <p className="text-[10px] text-text-m mt-0.5">{m.trigger}</p>
                        </div>
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full border flex-shrink-0 ${
                          activo
                            ? 'bg-ss-bg text-ss-text border-border-success'
                            : 'bg-surface-container text-text-m border-outline-variant'
                        }`}>
                          N{m.nivel} · {activo ? 'Activo' : 'Inactivo'}
                        </span>
                      </div>
                      <textarea
                        value={cfg[m.key] ?? ''}
                        onChange={e => setCfg(prev => ({ ...prev, [m.key]: e.target.value }))}
                        placeholder={m.placeholder}
                        rows={4}
                        disabled={!activo}
                        className="w-full bg-surface-container border border-outline-variant rounded-lg px-3 py-2 text-[11px] text-on-surface outline-none focus:border-stitch-primary resize-none disabled:opacity-40 placeholder:text-text-m/50"
                      />
                    </div>
                  )
                })}
              ...
              {saveError && <p className="text-[10px] text-se-text mt-4 mb-2">{saveError}</p>}
              <button onClick={guardar} disabled={saving}
                className="w-full mt-6 bg-gradient-to-r from-nt-navy to-blue-600 hover:brightness-110 disabled:opacity-60 text-white rounded-lg py-2.5 text-sm font-semibold transition-colors">
